...
dnorm_mod <- function(x, mean, sd, log = T){
  dnorm(x = x, mean = mean, sd = sd, log = log) - log(1 - pnorm(q = 0, mean = mean, sd = sd))
}
...
system.time({
  ...
  mcmcConf$removeSamplers(c("log_r"))
  mcmcConf$addSampler(target = c("log_K", "log_a", "log_H", "log_Q", "log_sigma", "x0"), 
                      type = 'RW_block')
  mcmcConf$getSamplers()
})
system.time({
...
abline(h = H)
plot(exp(samples[, 'log_Q']), type = "l", main = "Q")
abline(h = Q)
plot(exp(samples[, 'log_sigma']), type = "l", main = "sigma")
abline(h = sigma)
plot(samples[, 'x0'], type = "l", main = "x0")
abline(h = x0)
corrplot::corrplot(cor(samples[, c("log_K", "log_a", "log_H", "log_Q", "log_sigma", "x0")]))
```