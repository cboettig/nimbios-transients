...
```{r likelihood_surfaces_2D, cache = F}
...
```{r estimation_fix_a_K, cache = F}
## block sampler
system.time({
  mcmcConf <- configureMCMC(model)
  mcmcConf$getSamplers()
  mcmcConf$removeSamplers(c("log_a", "log_K"))
  mcmcConf$addSampler(target = c("log_r", "log_H", "log_Q", "log_sigma", "x0"), 
                      type = 'RW_block')
  mcmcConf$getSamplers()
})
system.time({
